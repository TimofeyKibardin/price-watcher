...
export async function scrapeWildberriesProduct(url: string) {
  ...
  try {
    // Получаем страницу с карточкой товара
    // const response = await axios.get(url, options);
    ...
    console.log("description: " + allArticles[0].description);
    ...
    const data = {
      ...
      originalPrice: Number(originalPrice) || Number(currentPrice),
      priceHistory: [],
      discountRate: (originalPrice && currentPrice) ? percentage(Number(currentPrice), Number(originalPrice)).toFixed() : '',
      category: category,
      reviewsCount: reviewsCount || '0',
      stars: stars || 'Нет оценок',
      // isOutOfStock: outOfStock,
